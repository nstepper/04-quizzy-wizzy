...
var totaltestQuestions = testQuestions.length;
...
function startTimer(){
    ...
    var timeInterval = setInterval(function(){
        ...
        if(secondsLeft <= 0 || (questionIndex > totaltestQuestions-1)){
...
function buildQuestion(){
    ...
    if(questionIndex > totaltestQuestions - 1){
        return;
    }
    else{
        ans =  testQuestions[questionIndex].answer;

        //Display Question 
        questionEl.innerHTML = testQuestions[questionIndex].title;
        questionEl.setAttribute("class","text-left");
        questionEl.style.display= "block";

        for(var j = 0 ; j < 4 ; j++){
            var index = choiceArray[j].getAttribute("data-index");
            choiceArray[j].textContent = (+index+1) +". "+testQuestions[questionIndex].choices[index];
